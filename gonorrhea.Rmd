...
race_df = 
  read_csv("./data/final_regression_variables_clean.csv") %>%
  rename(
  ...
  transform(
    other = paste(asian_pi + other_race)) %>% 
  select(-female, -total_pop, -asian_pi, -other_race, -foreign_born, -limited_eng, -unemployment, -clinics, -community_health_centers, -clinics_and_chc_count, -non_pcmd, -non_pcmd_density) %>% 
  reorder = c("nta_name", "nta_code", "black", "hispanic", "white", "other", "education", "poverty", "preterm_births", "smm", "gonorrhea", "health_ins", "medicaid_enroll", "late_no_prenatal_care", "clinics_and_chc_density")
...
gohorrea_black = 
  plots_df %>% 
  filter(race == "black_non_hisp") %>% 
  ggplot(aes(x = edu_less_than_hs, y = gonorrhea)) +
  geom_point()
